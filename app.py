...
def get_logfile_text() -> Optional[str]:
    """
    Function to load journalctl type log files and split into chunks
    aspanner to add content
    """
    st.header("Logfile upload")

    uploaded_file = st.file_uploader(
        label="Here, upload your log file you want OpenShift AiOps to use to answer",
        type="text"
    )
    if uploaded_file:
        pdf_reader = PdfReader(uploaded_file)
...
def select_llm() -> Union[ChatOpenAI, LlamaCpp]:
    ...
    #hardcoding model name as a protoype - change below aspanner-12Feb24 - removing UI options
    #model_name="llama-2-7b-chat.Q5_K_M.gguf"
    model_name="llama-2-7b-aiopsfinetuned-q8_0-gguf"

    temperature = 0.0
    
    aiOps_instance = st.sidebar.radio("Choose AIOps tuning for:",
                                      ("MicroShift",
                                      "RHDE (Linux only)",
...
def load_embeddings(model_name: str) -> Union[OpenAIEmbeddings, LlamaCppEmbeddings]:
    ...
    if model_name.startswith("gpt-"):
        ...
        return LlamaCppEmbeddings(model_path=f"./models/{model_name}")
...
def main() -> None:
    ...
    #texts = get_pdf_text() aspanner - change to logfile ingestion 13Feb24
    texts = get_logfile_text()
    qdrant = build_vectore_store(texts, embeddings)

    init_messages()

    st.header("OpenShift AiOps")
    ...
    if user_input := st.chat_input("Input your question!"):
        ...
        with st.spinner("OpenShift AIOps is typing ..."):
